 ...
 * MODIFICACIÓN: Migrado al catálogo universal 'productos' y columna 'id_producto'.
 * @author Sergio Mauricio Campos Carranza
 * @project Módulo Ventas DEMEX
 * @version 8.0 (Catálogo Universal de Productos)
 */
...
$id_producto          = isset($_POST['id_producto']) ? intval($_POST['id_producto']) : (isset($_POST['id_maquina']) ? intval($_POST['id_maquina']) : 0);
$cliente_razon        = trim($_POST['cliente'] ?? ''); 

// Si el RFC viene vacío del formulario de edición, asignamos el genérico oficial
...
// Captura de Fechas desde el Formulario de Edición
...
// Empaquetamos los datos bancarios en JSON codificado en base64
...
// Unimos las observaciones con el bloque cifrado bancario usando el delimitador (|||)
$notes_final = $notes_original . "|||" . $datos_bancos_empaquetados;

if ($id_cotizacion <= 0 || $id_producto <= 0 || empty($cliente_razon)) {
...
try {
    ...
    // Recálculo de vigencia del documento comercial
    ...
    // Obtener el nombre del producto desde la tabla central 'productos'
    $stmt_prod_name = $pdo->prepare("SELECT nombre FROM productos WHERE id_producto = ? LIMIT 1");
    $stmt_prod_name->execute([$id_producto]);
    $prod_info = $stmt_prod_name->fetch(PDO::FETCH_ASSOC);
    $producto_texto = $prod_info ? $prod_info['nombre'] : '';

    // 4. PASO A: Actualizar la tabla 'cotizacion' usando 'id_producto'
    ...
    // 5. PASO B: Sincronización del Lead (Solo si proviene de un prospecto)
    if (!empty($id_prospecto) && $id_prospecto > 0) {
        ...
        if ($id_formulario) {
            ...
            $stmt2->execute([
                ...
                ':maquina_interes' => $producto_texto,
