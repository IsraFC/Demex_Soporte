 ...
 * @version 1.5 - Soporte para Técnicos Asignados
 */
...
$id_usuario_activa = $_SESSION['id_usuario'];
...
$params = [':id_usuario_activo' => $id_usuario_activa];

if (!empty($searchValue)) {
    $where .= " AND (c.nombre_cliente LIKE :search OR t.no_serie LIKE :search OR tec.nombre LIKE :search) ";
...
$stmtTotal->execute([$id_usuario_activa]);
$totalRecords = $stmtTotal->fetchColumn();

$stmtFilter = $pdo->prepare("SELECT COUNT(*) $queryBase $where");
foreach ($params as $key => $val) { $stmtFilter->bindValue($key, $val); }
$stmtFilter->execute();
